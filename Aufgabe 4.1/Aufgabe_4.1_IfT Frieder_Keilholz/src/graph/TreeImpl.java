 ...
 * By default, the first node added when the tree is empty is the root node.
 * <p>
...
public class TreeImpl extends GraphImpl implements Tree {
     ...
     */
    public TreeImpl(boolean isDirected) {
        super(isDirected);
    }
    ...
    public Vertex getParent(Vertex vertex) throws GraphException {
        ...
        traversal.traverse(rootVertex, visited, new StopAtVisitor(vertex));

        // Now starting from the last element in the vertices that were visited,
        // find the first Vertex which is adjacent to the Vertex we are interested in.
     ...
     * @throws  EmptyTreeException is the <tt>Tree</tt> is empty.
     */
    public List<Vertex> getChildren(Vertex vertex) throws GraphException {
        // Check that root has been specified
        if (rootVertex == null) {
            throw new EmptyTreeException();
        }
        ...
        // minus its parent.
        List<Vertex> children = getAdjacentVertices(vertex);
	...
	public Tree getSubTree( Vertex subTreeRootVertex ) throws Exception {
        ...
        Edge    currentEdge;
        List<Edge>    incidentEdges = this.getEdges(subTreeRootVertex);
        for (int i = 0; i < incidentEdges.size(); i++) {
            currentEdge = incidentEdges.get(i);
        ...
        Tree    subTree = new TreeImpl(isDirected);
        ...
        for (int i = 0; i < visited.size(); i++) {
            // No need to add the Vertex manually, because
            // addEdge() will add it for us.
            currentVertex = visited.get(i);

            // Add the Vertex's Edges to the subtree, except for the
            // Edge leading to the parent of the subtree's root node.
            incidentEdges = (List<Edge>) ((ArrayList<Edge>)getEdges(currentVertex)).clone();
    ...
    public List<Vertex> getLeaves() {
        ...
        List<Vertex> leaves = new ArrayList<Vertex>( 10 );
        Vertex  currentVertex;

        for (int i = 0; i < size; i++) {
            currentVertex = (Vertex) this.vertices.get( i );
    ...
    public int getHeight() {
        List<Vertex> leaves = this.getLeaves();
    ...
    public boolean isPath(Vertex v1, Vertex v2) {
        ...
        if( v2 == (Vertex) visited.get( visited.size() - 1 ) )
            return true;
        else
            return false;
    }
    ...
    */
    ...
    public void removeEdges( Vertex v ) throws Exception {
        throw new NoSuchMethodException( "removeEdges( Vertex ) is not supported" );
    }
    */
}
